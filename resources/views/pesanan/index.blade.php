            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="grid grid-cols-10 gap-1">
                        {{-- <button onclick="window.location.replace('{{ route('pesanan.available') }}')"
                            ...
                            Tersedia</button>
                        <button onclick="window.location.replace('{{ route('pesanan.unavailable') }}')"
                        ...
                        <button type="button"
                            ...
                            onclick="window.location.replace('{{ route('pesanan.create') }}')">+ Tambah Pesanan</button>
                    </div>
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="grid grid-cols-3">
                                                <a href="{{ route('pesanan.edit', $product->id) }}"
                                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                                <a href="{{ route('pesanan.show', $product->id) }}"
                                                    class="ml-2 font-medium text-green-600 dark:text-green-500 hover:underline">Lihat</a>
                                                <form action="{{ route('pesanan.destroy', $product->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
